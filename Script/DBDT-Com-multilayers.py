#  -*-  coding: UTF-8  -*-
# This is a Python script for multilayers-DBDT-Composition package.
...
class SDTs(nn.Module):
    def __init__(
        ...
        for i in range(T):
            ...
            self.param_dict['W1' + str(i)] = nn.Parameter(
                torch.nn.init.xavier_uniform_(Variable(torch.randn(n_x, n_x + 1)).cuda(), gain=1))
            self.param_dict['b1' + str(i)] = nn.Parameter(
                torch.nn.init.constant_(Variable(torch.randn(1, n_x + 1)).cuda(), 0))
            self.param_dict['W2' + str(i)] = nn.Parameter(
                torch.nn.init.xavier_uniform_(Variable(torch.randn(n_x + 1, self.inner_numb)).cuda(), gain=1))
            self.param_dict['b2' + str(i)] = nn.Parameter(
